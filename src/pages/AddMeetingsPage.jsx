...
const AddMeetingsPage = () => {
    ...
    // Function to generate a random future date (1 year from now)
    ...
    const handleChange = (e) => {
        ...
        if (name === 'is_recurring') {
            ...
            setMeetingData(prev => ({
                ...prev,
                [name]: isRecurring,
                end_time: isRecurring ? generateFutureDate() : prev.end_time
            }));
        } else {
            setMeetingData(prev => ({ ...prev, [name]: value }));
        }
    ...
    const handleSubmit = async (e) => {
        ...
        const formattedStartTime = formatDateTime(meetingData.start_time, meetingData.hour, meetingData.minute, meetingData.ampm);
        const formattedEndTime = meetingData.is_recurring 
            ? formatDateTime(generateFutureDate(), meetingData.hour, meetingData.minute, meetingData.ampm)
            : formatDateTime(meetingData.end_time, meetingData.end_hour, meetingData.end_minute, meetingData.end_ampm);
        ...
        try {
            if (authToken && !isTokenExpired(authToken)) {
                ...
                toast.error("No valid authentication tokens found.");
            }
        } catch (err) {
            console.error("Error:", err);
            toast.error(err.message || "An error occurred.");
